...
settings.parm_filename         = 'C:\Users\schalklab\Box\McDonnel PTSD paradigm\parms\demovideo_Parameters.prm';
...
WM_faces([1 2]) = [];
face_nums = randperm(length(WM_faces),5);
for i = 1:5
    face_paths(i,:) = [WM_faces(face_nums(i)).folder '\' WM_faces(face_nums(i)).name];
end
% face 5 is dummy face for extinction

fc = dir('C:\Users\schalklab\Box\McDonnel PTSD paradigm\Stimuli\fixation crosses');
fc([1 2]) = []; % 1 is down 2 is left 3 is right 4 is up
for i = 1:4
...
scales(1:3) = [];
for i = face_nums
    likert_paths(i,:) = [scales(face_nums(i)).folder '\' scales(face_nums(i)).name];
end
...
param.Stimuli.Value{4,idx}      = '5s'; %stimulusDuration
param.Stimuli.Value{5,idx}      = '0'; %audioVolume
param.Stimuli.Value{6,idx}      = 'cross'; %category
param.Stimuli.Value{7,idx}      = 'GazeHeld >= 500'; %EarlyOffsetExpression
...
param.Stimuli.Value{5,idx}      = '0'; %audioVolume
param.Stimuli.Value{6,idx}      = 'cross'; %category
param.Stimuli.Value{7,idx}      = 'GazeHeld >= 1500'; %EarlyOffsetExpression
...
param.Stimuli.Value{5,idx}      = '0'; %audioVolume
param.Stimuli.Value{6,idx}      = 'cross'; %category
param.Stimuli.Value{7,idx}      = 'GazeHeld >= 2500'; %EarlyOffsetExpression
...
param.Stimuli.Value{5,idx}      = '0'; %audioVolume
param.Stimuli.Value{6,idx}      = 'cross'; %category
param.Stimuli.Value{7,idx}      = 'GazeHeld >= 3500'; %EarlyOffsetExpression
...
seq = [301 201 14 1 13 2 12 3 11 4 202 301]';
