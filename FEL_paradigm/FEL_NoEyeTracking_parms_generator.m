...
blocknames = cell(1,8);
blocknames{1,1} = fullfile(settings.BCI2000path,'McDonnel PTSD paradigm','parms','FEL_NoEyeTracking_Habituation_Parameters.prm');
blocknames{1,2} = fullfile(settings.BCI2000path,'McDonnel PTSD paradigm','parms','FEL_NoEyeTracking_Acquisition_1_Parameters.prm');
blocknames{1,3} = fullfile(settings.BCI2000path,'McDonnel PTSD paradigm','parms','FEL_NoEyeTracking_Acquisition_2_Parameters.prm');
blocknames{1,4} = fullfile(settings.BCI2000path,'McDonnel PTSD paradigm','parms','FEL_NoEyeTracking_Acquisition_3_Parameters.prm');
blocknames{1,5} = fullfile(settings.BCI2000path,'McDonnel PTSD paradigm','parms','FEL_NoEyeTracking_Extinction_Parameters.prm');
blocknames{1,6} = fullfile(settings.BCI2000path,'McDonnel PTSD paradigm','parms','FEL_NoEyeTracking_Recall_1_Parameters.prm');
blocknames{1,7} = fullfile(settings.BCI2000path,'McDonnel PTSD paradigm','parms','FEL_NoEyeTracking_Recall_2_Parameters.prm');
blocknames{1,8} = fullfile(settings.BCI2000path,'McDonnel PTSD paradigm','parms','FEL_NoEyeTracking_Recall_3_Parameters.prm');
blocknames{1,9} = fullfile(settings.BCI2000path,'McDonnel PTSD paradigm','parms','FEL_NoEyeTracking_Briefing_Parameters.prm');

% Get task images
WM_faces = dir(strcat(localPath,'\Box\McDonnel PTSD paradigm\Stimuli\face slides'));
WM_faces([1 2]) = [];
...
fc = dir(strcat(localPath,'\Box\McDonnel PTSD paradigm\Stimuli\fixation crosses'));
fc([1 2]) = []; % 1 is left 2 is right
for i = 1:2
    fc_paths(i,:) = [fc(i).folder '\' fc(i).name];
end

scales = dir(strcat(localPath,'\Box\McDonnel PTSD paradigm\Stimuli\likert'));
scales([1 2]) = [];
for i = 1:4
...
% Set up ChangeFixation
%
param.ChangeFixation.Section         = 'Application';
param.ChangeFixation.Type            = 'matrix';
param.ChangeFixation.DefaultValue    = '';
param.ChangeFixation.LowRange        = '';
param.ChangeFixation.HighRange       = '';
param.ChangeFixation.Comment         = 'Change fixation centers during a run';
param.ChangeFixation.Value           = cell(n_stimuli, 3);
param.ChangeFixation.Value(:)        = {''};
param.ChangeFixation.RowLabels       = cell(n_stimuli, 1);
param.ChangeFixation.ColumnLabels    = cell(1, 3);
param.ChangeFixation.RowLabels(:)    = {''};
param.ChangeFixation.ColumnLabels(:) = {''};
...
for idx = 1:5
    ...
    param.ChangeFixation.Value{idx,1} = ['StimulusCode==' num2str(idx)];
    param.ChangeFixation.Value{idx,2} = '0.5';
    param.ChangeFixation.Value{idx,3} = '0.5';
end
...
% all fixation crosses are between 12-83
% blanks 12:29
% 3 +/- 1s
...
% fixation cross left white 30:47
% 3 +/- 1s
SamplingRate = str2double(settings.SamplingRate(1:end-2));
BlockSize    = str2double(settings.SampleBlockSize);
MinDuration  = 2;
...
for idx = 30:47
    ...
    param.Stimuli.ColumnLabels{idx} = sprintf('%d',idx);
    param.Stimuli.Value{1,idx}      = '';
    param.Stimuli.Value{2,idx}      = fc_paths(1,:);
    param.Stimuli.Value{3,idx}      = '';
    param.Stimuli.Value{4,idx}      = [num2str(duration) 's'];
    param.Stimuli.Value{5,idx}      = '0';      
    param.Stimuli.Value{6,idx}      = 'fixation'; 
    param.Stimuli.Value{7,idx}      = 'KeyDown == 32';
    param.Stimuli.Value{8,idx}      = '0';
    param.ChangeFixation.Value{idx,1} = ['StimulusCode==' num2str(idx)];
    param.ChangeFixation.Value{idx,2} = '0.1';
    param.ChangeFixation.Value{idx,3} = '0.5';
end

% fixation cross right white 48:65
% 3 +/- 1s
BlockSize    = str2double(settings.SampleBlockSize);
MinDuration  = 2;
MaxDuration  = 4;
for idx = 48:65
    ...
    param.Stimuli.ColumnLabels{idx} = sprintf('%d',idx);
    param.Stimuli.Value{1,idx}      = '';
    param.Stimuli.Value{2,idx}      = fc_paths(2,:);
    param.Stimuli.Value{3,idx}      = '';
    param.Stimuli.Value{4,idx}      = [num2str(duration) 's'];
    param.Stimuli.Value{5,idx}      = '0';      
    param.Stimuli.Value{6,idx}      = 'fixation'; 
    ...
    param.Stimuli.Value{8,idx}      = '0';
    param.ChangeFixation.Value{idx,1} = ['StimulusCode==' num2str(idx)];
    param.ChangeFixation.Value{idx,2} = '0.9';
    param.ChangeFixation.Value{idx,3} = '0.5';
end
...
% % fixation cross right green 66:83
% % variable duration from 2.25-2.75s to incorporate 0.5s for arrow
% SamplingRate = str2double(settings.SamplingRate(1:end-2));
% BlockSize    = str2double(settings.SampleBlockSize);
% MinDuration  = 0.5;
% MaxDuration  = 1.2;
...
%     duration  = blockvals(randval);
%     
%     param.Stimuli.ColumnLabels{idx} = 'FC r g';
%     param.Stimuli.Value{1,idx}      = '';
%     param.Stimuli.Value{2,idx}      = fc_paths(3,:);
%     param.Stimuli.Value{3,idx}      = '';
%     param.Stimuli.Value{4,idx}      = '2s';
%     param.Stimuli.Value{5,idx}      = '0';      
%     param.Stimuli.Value{6,idx}      = 'fixation'; 
%     param.Stimuli.Value{7,idx}      = ['FixationDuration >= ' num2str(1000*duration)];
%     param.Stimuli.Value{8,idx}      = '0';
%     param.ChangeFixation.Value{idx,1} = ['StimulusCode==' num2str(idx)];
%     param.ChangeFixation.Value{idx,2} = '0.9';
%     param.ChangeFixation.Value{idx,3} = '0.5';
...
idx = 86;
param.Stimuli.ColumnLabels{idx} = sprintf('%d',idx);
param.Stimuli.Value{1,86}      = 'This block is complete. Please take a short break. Press space when you are ready to continue.'; %caption
param.Stimuli.Value{2,86}      = '';
param.Stimuli.Value{3,86}      = ''; %audio
param.Stimuli.Value{4,86}      = '600s'; %stimulusDuration
param.Stimuli.Value{5,86}      = '0'; %audioVolume
param.Stimuli.Value{6,86}      = 'instruction'; %category
param.Stimuli.Value{7,86}      = 'KeyDown == 32';
param.Stimuli.Value{8,86}      = '0'; %EstimOn
...
% Acquisition Sequence 1
    ...
    likert_seq = write_likert_seq(1:4);
    seq_acq1 = [seq_acq1 likert_seq 85 87]';
...
% Acquisition Sequence 2
    ...
    likert_seq = write_likert_seq(1:4);
    seq_acq2 = [seq_acq2 likert_seq 85 87]';
...
% Acquisition Sequence 3
    ...
    likert_seq = write_likert_seq(1:4);
    seq_acq3 = [seq_acq3 likert_seq 85 87]';
...
% Extinction Sequence
    ...
    seq_ext = [seq_ext likert_seq 85 87]';
...
% Recall Sequence 1
    ...
    likert_seq = write_likert_seq(1:4);
    seq_rec1 = [seq_rec1 likert_seq 85 87];
...
% Recall Sequence 2
    ...
    likert_seq = write_likert_seq(1:4);
    seq_rec2 = [seq_rec2 likert_seq 85 87];
...
% Recall Sequence 3
    ...
    likert_seq = write_likert_seq(1:4);
    seq_rec3 = [seq_rec3 likert_seq 85 87];
...
param.VisualizeGazeMonitorFilter.DefaultValue    = '1';
...
%
param.EnforceFixation.Section         = 'Application';
param.EnforceFixation.Type            = 'int';
param.EnforceFixation.DefaultValue    = '1';
param.EnforceFixation.LowRange        = '0';
param.EnforceFixation.HighRange       = '1';
param.EnforceFixation.Comment         = 'Enforce a fixation point on the screen';
param.EnforceFixation.Value           = {'1'};

%
param.ShowFixationZone.Section         = 'Application';
param.ShowFixationZone.Type            = 'int';
param.ShowFixationZone.DefaultValue    = '1';
param.ShowFixationZone.LowRange        = '0';
param.ShowFixationZone.HighRange       = '1';
param.ShowFixationZone.Comment         = 'Show the legal fixation zone with a ring';
param.ShowFixationZone.Value           = {'1'};

%
param.LogGazeInformation.Section         = 'Application';
param.LogGazeInformation.Type            = 'int';
param.LogGazeInformation.DefaultValue    = '1';
param.LogGazeInformation.LowRange        = '0';
param.LogGazeInformation.HighRange       = '1';
param.LogGazeInformation.Comment         = 'Add Eyetracker information to applog';
param.LogGazeInformation.Value           = {'1'};
...
function [cross_1 cross_2] = fcgenerator(number)
switch number
    case 1
        cross_1 = randi(18)+11;
        cross_2 = randi(18)+29;
