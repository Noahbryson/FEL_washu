...
settings.InstructionsCaption   = {'Stimulus Titration. Press space to continue'; 'End of block. Please rest.'};

%% Get task images
task_images = dir(fullfile(settings.BCI2000path,'prog','images','*.bmp'));
pain_scale = 'C:\Users\schalklab\Box\McDonnel PTSD paradigm\Stimuli\pain-scale.jpg';
...
for idx = 2:3
    param.Stimuli.ColumnLabels{idx} = sprintf('%d',idx);
    param.Stimuli.Value{1,idx}      = settings.InstructionsCaption{idx-1};
    param.Stimuli.Value{2,idx}      = '';
    param.Stimuli.Value{3,idx}      = '';
...
%% Shock administration on 4
idx = 4;
param.Stimuli.ColumnLabels{idx} = sprintf('%d',idx);
param.Stimuli.Value{1,idx}      = 'Stimulator ON';
param.Stimuli.Value{2,idx}      = '';
param.Stimuli.Value{3,idx}      = '';
param.Stimuli.Value{4,idx}      = '0.1s';
param.Stimuli.Value{5,idx}      = '0';      
param.Stimuli.Value{6,idx}      = 'stimON'; 
param.Stimuli.Value{7,idx}      = '';
param.Stimuli.Value{8,idx}      = '1';

%% Shock administration off 5
idx = 5;
param.Stimuli.ColumnLabels{idx} = sprintf('%d',idx);
param.Stimuli.Value{1,idx}      = 'Stimulator OFF';
param.Stimuli.Value{2,idx}      = '';
param.Stimuli.Value{3,idx}      = '';
param.Stimuli.Value{4,idx}      = '0.9s';
param.Stimuli.Value{5,idx}      = '0';      
param.Stimuli.Value{6,idx}      = 'stimOFF'; 
param.Stimuli.Value{7,idx}      = '';
param.Stimuli.Value{8,idx}      = '0';
...
param.Stimuli.ColumnLabels{idx} = sprintf('%d',idx);
param.Stimuli.Value{1,idx}      = '';
param.Stimuli.Value{2,idx}      = pain_scale;
param.Stimuli.Value{3,idx}      = '';
param.Stimuli.Value{4,idx}      = '60s';
param.Stimuli.Value{5,idx}      = '0';      
param.Stimuli.Value{6,idx}      = 'sync'; 
param.Stimuli.Value{7,idx}      = 'KeyDown == 48 || KeyDown==49 || KeyDown==50 || KeyDown==51 || KeyDown==52 || KeyDown==53 || KeyDown==54 || KeyDown==55 || KeyDown==56 || KeyDown==57';
param.Stimuli.Value{8,idx}      = '0';
%% Sequence

seq = [1 2 5 5 4 5 5 6 5 5 4 5 5 6 5 5 4 5 5 6 3 1]';
...
settings.parm_filename = fullfile(settings.BCI2000path,'McDonnel PTSD paradigm','parms','Tao_FEL_titration.prm');
